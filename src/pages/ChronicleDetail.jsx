...
export default function ChronicleDetail() {
  ...
  // Find chronicle by slug (Unicode/Nepali supported) or fallback by ID
  const targetSlug = decodeURIComponent(slug || '').toLowerCase();
  const chronicle = chroniclesList.find(
    c => (c.slug && (c.slug.toLowerCase() === targetSlug || decodeURIComponent(c.slug).toLowerCase() === targetSlug)) || String(c.id) === String(slug)
  ) || chroniclesList[0];
