...
export default function EventDetail() {
  ...
  // Find event by slug (Unicode/Nepali supported) or fallback by ID
  const targetSlug = decodeURIComponent(slug || '').toLowerCase();
  const event = eventsList.find(
    e => (e.slug && (e.slug.toLowerCase() === targetSlug || decodeURIComponent(e.slug).toLowerCase() === targetSlug)) || String(e.id) === String(slug)
  ) || eventsList[0];
    ...
    <div style={{ minHeight: '100vh', paddingBottom: '100px', backgroundColor: '#f8fafc' }}>
      ...
      <div className="rns-container" style={{ paddingTop: '50px', maxWidth: '880px' }}>
        ...
        <article className="rns-card" style={{ padding: '40px', backgroundColor: '#ffffff', borderRadius: '24px' }}>
          ...
          <FormattedHtml content={event.content || event.excerpt || 'No description available for this campus event.'} />
