...
export function CampusProvider({ children }) {
  const [data, setData] = useState(() => {
    try {
      ...
      if (saved) {
        ...
        if (parsed) {
          if (!parsed.chronicles || !Array.isArray(parsed.chronicles) || parsed.chronicles.length === 0) {
            parsed.chronicles = defaultData.chronicles;
          }
    ...
    return {
      ...
      chronicles: defaultData.chronicles
    };
  });
  ...
  // Sync dataset and PDFs from IndexedDB on initial mount
  useEffect(() => {
  ...
  const updateEvents = (newEvents) => {
    saveAllCampusData({ eventsList: newEvents });
  };
  ...
  const updateChronicles = (newChronicles) => {
    saveAllCampusData({ chroniclesList: newChronicles });
  };
