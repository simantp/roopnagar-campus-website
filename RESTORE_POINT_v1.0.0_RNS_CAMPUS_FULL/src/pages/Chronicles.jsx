...
export default function Chronicles() {
  ...
  const getExcerpt = (item) => {
    if (item.excerpt && item.excerpt.trim()) {
      const clean = item.excerpt.trim();
      return clean.length > 160 ? clean.slice(0, 157).trim() + '...' : clean;
    }
    const rawText = item.content || (item.fullContent ? item.fullContent.join(' ') : '');
    const textOnly = rawText.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
    if (!textOnly) return 'Read full chronicle article for complete details.';
    return textOnly.length > 160 ? textOnly.slice(0, 157).trim() + '...' : textOnly;
  };
